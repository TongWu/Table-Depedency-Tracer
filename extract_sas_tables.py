...
Studio jobs. It locates ``_INPUT``/``_OUTPUT`` macro assignments to recover the
upstream (source) and downstream tables referenced by a script.
...
def extract_summary(path: Path) -> TableSummary:
    ...
    outputs: List[str] = []
    for prefix in sorted(output_tables.keys()):
        table = output_tables[prefix]
        key = table.lower()
...
def render_summary(summary: TableSummary) -> str:
    ...
    if summary.outputs:
        lines.extend(f"    - {tbl}" for tbl in summary.outputs)
    else:
        lines.append("    (none detected)")
    return "\n".join(lines)
